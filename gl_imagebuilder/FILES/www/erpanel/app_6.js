...
step_activate = function(step_id) {
  step_array[step_id] = true;
  logs_log("jumanji step " + step_id);
  mqtt_publish(step_id.toString(), "/er/jumanji/step");
}
...
langset = function(lang) {
  ...
  var langStr = "English";
  if      (lang == 2) langStr = "Language 2";
  else if (lang == 3) langStr = "Language 3";
  else if (lang == 4) langStr = "Language 4";
  else if (lang == 5) langStr = "Language 5";
...
timeset = function(time) {
  ...
  var game_period_min;
  if (time == 1) game_period_min = 60;
  if (time == 2) game_period_min = 55;
  if (time == 3) game_period_min = 50;
  if (time == 4) game_period_min = 45;
  if (time == 5) game_period_min = 30;
  if (time == 6) game_period_min = 1;
  if (time == 7) game_period_min = 70;
  if (time == 8) game_period_min = 80;
...
var options = {
  ...
  onSuccess: function() {
    ...
    mqtt_subscribe("/er/jumanji/step_show");
    mqtt_subscribe("/er/jumanji/step_finish");
  },
...
var jumanji_show_step_btns = false;
var step_array = [false, false, false, false, false, false, false, false, false, false, false, false];
...
var msgs_check = function(topicStr, dataStr) {
  ...
  } else if (topicStr == "/er/jumanji/step_finish") {
    step_array[dataStr] = true;
    // logs_log(dataStr);
    // var array = dataStr.split(',');
    // for (let i = 0; i < array.length; i++){
    //   var ar_ind = array[i];
    //   logs_log(ar_ind);
    //   step_array[ar_ind] = true;
    // }
    // array.forEach((element) => {
    //   step_array[element] = true;
    // });
    logs_log(step_array);
    logs_log(step_array[3]);
    //step_array[6] == undefined
    // step_array = 
    // if (dataStr == "true") {
    //   jumanji_show_step_btns = true;
    }
...
var ui_riddles_show = function() {
  ...
  if(riddlesInfo[i].strName == "Table1") {
    strItem += step_btns(1);
  }
  if(riddlesInfo[i].strName == "Table2") {
    strItem += step_btns(2);
  }
  if(riddlesInfo[i].strName == "Table3") {
    strItem += step_btns(3);
  }
...
step_btns = function(table_index) {
  var index = 4 * (parseInt(table_index) - 1);
  var strBtn = '<div class="row" >';
  var animals = ["monkey", "rhino", "lion", "elephant","monkey", "rhino", "lion", "elephant","monkey", "rhino", "lion", "elephant"];
  for (let i = index; i < index + 4; i++) {
    var color = step_array[i] ? "green": "orange";
    strBtn += '<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12" style="padding: 10px;">';
    strBtn +=   '<button id="step' + i +'" class="ds-btn ds-btn-' + color + '" onclick=\'animateClick(this, "ds-btn-click"); step_activate(' + i + ');\' style="font-size: 16px;" >' + animals[i] +'</button>';
    strBtn += '</div>';
  }
  strBtn += '</div>';
  return strBtn;
...
var ui_soundlist_show = function() {
  ...
  for (var i = 0; i < soundList.length; i++) {
    ...
    var strItem = '<br><button class="btn-sound text-left" type="button" onclick=\'animateClick(this, "ds-btn-click");ui_onBtnSoundPlay("' + soundList[i].strId + "\");'>" + strNumber + soundList[i].strName + "</button>";
    strResult += strItem;
  }
